...
func SetTagHelper(t *testing.T, source string) {
	out := tempFileName(t)
	...
	bin, err := NewBinary(input)
	if err != nil {
		t.Fatalf("read failed: %v", err)
	}
	f2, err := os.OpenFile(out, os.O_CREATE|os.O_RDWR, 0644)
	if err != nil {
		t.Fatalf("create failed: %v", err)
	}
	err = bin.SetTag(f2, []byte(expected))
	f2.Close()
	if err != nil {
...
func (bin MSIBinary) Validate(other *MSIBinary) (bool, error) {
	...
	cert, index, err := getTagCert(bin.signedData)
	if err != nil {
		return false, fmt.Errorf("parse error in bin.signedData: %w", err)
	}
	if other != nil {
		_, index2, err := getTagCert(other.signedData)
		if err != nil {
...
func TestMsiSuperfluousCert(t *testing.T) {
	...
	for _, e := range expect {
		...
		err = bin.SetTag(&buf, []byte(tag))
		if err != nil {
			t.Errorf("Error tagging test input %s: %v", e.infile, err)
			continue
		}
		binTagged, err := NewBinary(buf.Bytes())
		if err != nil {
